...
export default function TokenItem({ token, index, onSelect, isSelected }: TokenItemProps) {
  ...
  return (
    <div 
      className={`flex w-full h-[80px] border-b border-[#DFDFDF] 
                  ${isSelected ? 'bg-[#333096]' : 'bg-white hover:bg-[#333096]'}
    ...
    >
      ...
      <div className="w-[310px] h-full border-r border-[#DFDFDF] flex items-center px-4">
        <div className="flex items-center gap-4">
          <div className="w-[64px] h-[64px] bg-[#C6C6C6] border-2 border-[#848584] p-[2px]">
            <Image
              src={token.image}
              alt={token.name}
              width={60}
      ...
      <div className="w-[80px] h-full flex items-center justify-center">
        <button
          ...
          className={`w-[16px] h-[16px] bg-white border-none relative
                    shadow-[inset_-1px_-1px_0px_#FFF,inset_1px_1px_0px_#808080,inset_-2px_-2px_0px_#C1C1C1,inset_2px_2px_0px_#000]`}
        >
          {isSelected && (
